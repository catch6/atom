...
throw new BusinessException(Code.INVALID_PARAMETER);
```
...
// 关键业务失败
throw new ServiceException("短信发送失败", 5001);
```
...
// 集合校验
Must.notEmpty(list, "列表不能为空");
Must.noNullElements(array, "数组不能包含空元素");

// 业务逻辑校验
Must.isEquals(password, confirmPassword, "两次密码输入不一致");
Must.notContains(email, "@spam.com", "不支持该邮箱域名");
```
...
Should.matches(email, "^[\\w.-]+@[\\w.-]+\\.[a-zA-Z]{2,}$", "邮箱格式不正确");
```
